...
SIMULATOR::SIMULATOR(int numActions, int numObservations, double discount, std::string type)
...
bool SIMULATOR::LocalMove(STATE& state,
    int stepObs, const STATUS& status) const
{
    return true;
}

py::array_t<int> SIMULATOR::GenerateLegal() const
{   
    vector<int> actions;
...
void SIMULATOR::GeneratePreferred(const STATE& state, 
    ...
    const STATUS& status) const
{
    ...
    {
        actions.clear();
        GeneratePreferred(state, actions, status);
        if (!actions.empty())
...
void SIMULATOR::Prior(const STATE* state,
...
void SIMULATOR::resetStartPos()
{

}
COORD SIMULATOR::getStartPos()
{
return COORD(0,0);
}
void SIMULATOR::setStartPos(COORD iPos)
{
